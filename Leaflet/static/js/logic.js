...
let map = L.map("map", {
  center: [37.09, -119.5], // Center over CA
  zoom: 5,
...
// Create an object to hold the overlays.
let overlayMaps = {
...
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_day.geojson").then(function (data) {
  ...
  legend.onAdd = function () {
    ...
    for (let i = 0; i < depthIntervals.length; i++) {
      div.innerHTML += "<i style='background: " + colors[i] + "'></i> " + depthIntervals[i] + (depthIntervals[i + 1] ? "&ndash;" + depthIntervals[i + 1] + "<br>" : "+"); 
    }
    return div;
  };
